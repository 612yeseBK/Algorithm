...
public class HamiltonCircuit {
     ...
     * @param step
     */
        public void dfs(int[][] adjMatrix, int[] path, boolean[] used, int step) {
...
//                    System.exit(0);
                ...
                for(int i = 0;i < adjMatrix.length;i++) {
                    if(!used[i] && adjMatrix[path[step - 1]][i] == 1) {
                        used[i] = true;
                        path[step] = i;
                        dfs(adjMatrix, path, used, step + 1);
                        used[i] = false;    //进行回溯处理,无论成功失败都去寻找下一个路径
                        path[step] = -1;
                    }
                }
            }
        ...
        @Test
        public void hamiltonCircuitTest() {
